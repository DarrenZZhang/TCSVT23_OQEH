...
class MSCOCO_step2_eval:
    def __init__(self,
        ...
        # Fetch data
        with open(os.path.join(root, 'retrieval_file_list.txt'), 'r') as image_file:
            database_data = np.array([i.strip() for i in image_file])
        database_targets = np.loadtxt(os.path.join(root, 'retrieval_label.txt'), dtype=np.int8)

        with open(os.path.join(root, 'test_file_list.txt'), 'r') as image_file:
            test_data = np.array([i.strip() for i in image_file])
        test_targets = np.loadtxt(os.path.join(root, 'test_label.txt'), dtype=np.int8)

        # one views
        self._database_dataset = ImgDataset(data=database_data,
                                            targets=database_targets,
                                            transform=test_transforms,
                                            img_root=img_root) # train set
        self._test_dataset = ImgDataset(data=test_data,
                                        targets=test_targets,
                                        transform=test_transforms,
                                        img_root=img_root2) # val set
